...
async def create_user(user_data: UserCreate, db: Session = Depends(get_db)):
    return user_services.create(user_data, db)
...
async def read_user(user_id: int, db: Session = Depends(get_db)):
    return user_services.get_by_id(user_id, db)
...
Depends(get_db)):
    return user_services.update(user_id, updated_user, db)


@user_route.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_user(user_id: int, db: Session = Depends(get_db)):
    return user_services.destroy(user_id, db)
